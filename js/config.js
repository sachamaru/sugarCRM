...
;
Prism.hooks.add("after-highlight",function(e){var t=e.element.parentNode;if(!t||!/pre/i.test(t.nodeName)||t.className.indexOf("line-numbers")===-1){return}
	//var n=1+e.code.split("\n").length;
	var linesInTokenStack = 0;
	if (e.tokenStack) {
		linesInTokenStack = e.tokenStack.join().split("\n").length-1;
	}
	var n = (1 + e.code.split('\n').length + linesInTokenStack);
	var r;lines=new Array(n);lines=lines.join("<span></span>");r=document.createElement("span");r.className="line-numbers-rows";r.innerHTML=lines;if(t.hasAttribute("data-start")){t.style.counterReset="linenumber "+(parseInt(t.getAttribute("data-start"),10)-1)}e.element.appendChild(r)})
;


Prism.languages.shell = Prism.languages.bash;
},{}],3:[function(require,module,exports){
window.addEventListener('load', require('./main'), false);
},{"./main":5}],4:[function(require,module,exports){
module.exports = (function(){
    var meta = document.createElement('meta');
    meta.setAttribute('name', 'google-site-verification');
    meta.setAttribute('content', 'Ty6_PWZJdO7A6jwnkYj1axry76GI1h-CuY81TrY9mbc');
    document.querySelector('head').appendChild(meta);


    (function() {
      var cx = '007718218132233243975:ul-b5k2dwgm';
    ...
    var searchbox = function() {
      ...
      var amnt = 0;
      var boxes = document.getElementsByClassName('sugar-search');
      if (boxes.length > 1) {
        ++amnt;
      }
      boxes[amnt].innerHTML = '<div class="gcse-searchbox-only" \
          data-resultsURL="//support.sugarcrm.com/search/" ></div>';

      if ('placeholder' in input) {
        setTimeout(function() {
          if ($('.home').length > 0)
            $('.sugar-search input[type="text"]').attr('placeholder', 'Find documentation, videos, training and more...');
        }, 1000);
      }

      $('.sugar-search').css({'display': 'block'});
    }

    $(document).ready(searchbox);
})();
},{}],5:[function(require,module,exports){
var Widgets, bootstrap, gcse, kbview, prism, referenced, script, sugar_widgets, toc, views, w, widgets, widgets_include, _i, _len;

bootstrap = require('../deps/bootstrap/bootstrap-3.1.1');

window.one = require('./widgets/one');

window.two = require('./widgets/two');

window.three = require('./widgets/three');

Widgets = require('./widgets');

sugar_widgets = new Widgets('#widgets');
...
widgets_include = $('#widgets').data('widgets');

if (widgets_include && widgets_include.length > 0) {
  for (_i = 0, _len = widgets_include.length; _i < _len; _i++) {
    w = widgets_include[_i];
    if (widgets[w]) {
      sugar_widgets.add([widgets[w]]);
    }
  }
}

referenced = require('./views/referenced');

views = require('./views/views');

kbview = require('./scripts/kbview');

toc = require('./scripts/toc');

script = document.createElement('script');

script.src = "/assets/js/scripts/navtree.js";

document.body.appendChild(script);

prism = require('../deps/prism/prism');

gcse = require('./gcse');


},{"../deps/bootstrap/bootstrap-3.1.1":1,"../deps/prism/prism":2,"./gcse":4,"./scripts/kbview":6,"./scripts/toc":7,"./views/referenced":8,"./views/views":9,"./widgets":10,"./widgets/one":11,"./widgets/three":12,"./widgets/two":13}],6:[function(require,module,exports){
$(document).ready(function() {
  return $('.mtscript').each(function() {
    var proxyURL, view;
    view = $(this).data('script');
    if (view === 'kbview') {
      proxyURL = $(this).data('proxyurl');
      proxyURL += '&format=json';
      $(this).removeClass('hide').append($(document.createElement('div')).attr('class', 'kbcontent').attr('id', 'trending').append($(document.createElement('h2')).html('Trending')));
      return $.ajax({
        url: proxyURL,
        dataType: 'jsonp',
            ...
            if (i !== 0) {
              ...
              return $.ajax({
                ...
                success: function(pageContent) {
                  var li, link, title;
                  title = $(pageContent).find('#main h1').html();
                  link = $(document.createElement('a')).html(title).attr('href', url);
                  li = $(document.createElement('li'));
                  li.append(link);
                  li.append($(document.createElement('span')).html(' (' + pagecount + ')'));
                  return ul.append(li);
                }
              });
            }
...
$(document).ready(function() {
  return $('.mtscript').each(function() {
    var div, existingIds, h3, ul, view, widg;
    view = $(this).data('script');
    existingIds = [];
    if (view === 'toc') {
      widg = $(document.createElement('div')).attr('id', 'TableOfContents');
      div = $(document.createElement('div')).attr('class', 'widget');
      h3 = $(document.createElement('h3')).html('Table of Contents');
      ul = $(document.createElement('ul'));
      $('section h1, section h2, section h3, section h4, section h5, section h6').each(function() {
        var a, hide, level, li, link, newId, title;
        title = $(this).text();
        title = title.replace(/&nbsp;/g, ' ');
        level = $(this).get(0).nodeName.substring(1, 2);
        $(this).attr('data-level', level);
        link = $(this).html();
        link = link.replace(/&nbsp;/g, "_");
        link = link.replace(new RegExp(" ", 'g'), "_");
        newId = link;
        if (link in existingIds) {
          existingIds[link] = existingIds[link] + 1;
          newId = link + '_' + existingIds[link];
        } else {
          existingIds[link] = 1;
        }
        $(this).attr('id', newId);
        link = newId;
        a = $(document.createElement('a')).attr('href', '#' + link).attr('data-level', level).html(title);
...
$(document).ready(function() {
  return $('.referenced_content').each(function() {
    var url,
      _this = this;
    url = $(this).data('rel');
    if (url) {
      return $(this).load(url + ' #main', function(r, s, xhr) {
        return $(_this).html($(_this).children('#main').html());
      });
    }
  });
});
...
$(document).ready(function() {
  var getData;
  getData = function(dir, title, paragraph) {
    $.ajax({
      url: dir + '/',
      dataType: 'html',
      success: function(data) {
        ...
        title.text(HTML.find("#main h1").first().text());
        paragraph.html(HTML.find("#main p").first().text());
      },
      error: function() {
        paragraph.html("Failed to load content.");
      }
    });
  };
  $('[data-function]').each(function() {
    var container, d, directories, dirs, item, link, list, para, title, view, views, _i, _j, _len, _len1;
    view = $(this).data('function');
    views = view.split(',');
    for (_i = 0, _len = views.length; _i < _len; _i++) {
      view = views[_i];
      if (view === '/IDF/Views/Guide') {
        dirs = $(this).data('dirs');
        directories = dirs.split(',');
        container = $(document.createElement('div')).attr('class', 'view-container');
        title = $(document.createElement('h2')).html('Topics');
...
Widgets = (function() {
  ...
  Widgets.prototype.add = function(widgets) {
    var widget, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = widgets.length; _i < _len; _i++) {
      widget = widgets[_i];
      widget.build();
      _results.push(widget.append(this.id));
    }
    return _results;
  };

  Widgets.prototype.remove = function(widgets) {
    var widget, _i, _len, _results;
    _results = [];
    for (_i = 0, _len = widgets.length; _i < _len; _i++) {
      widget = widgets[_i];
      _results.push($('#' + widget).remove());
    }
    return _results;
  };
...
one.contents([
  {
    ...
    'href': '/02_Documentation/01_Sugar_Editions'
  }, {
    'name': 'Mobile Documentation',
    'href': '/02_Documentation/02_Mobile_Solutions'
  }, {
    'name': 'Plug-in Documentation',
    'href': '/02_Documentation/03_Plug-ins'
  }, {
    'name': 'Installable Connectors',
    'href': '/02_Documentation/035_Installable_Connectors'
  }, {
    'name': 'Developer Guide',
    'href': '/02_Documentation/04_Sugar_Developer'
  }, {
    'name': 'Supported Versions',
    'href': '/05_Resources/02_Supported_Versions'
  }, {
    'name': 'Supported Platforms',
    'href': '/05_Resources/03_Supported_Platforms'
  }, {
    'name': 'Accessibility',
    'href': '/05_Resources/035_Accessibility'
  }
]);
...
},{"./widget":14}],12:[function(require,module,exports){
var Widget, resources;

Widget = require('./widget');

resources = new Widget('resources');

resources.setTitle('Customer Center');

resources.contents([
  {
    'name': 'My Account',
    'href': 'https://web.sugarcrm.com/account'
  }, {
    'name': 'Purchased Downloads',
    'href': 'https://store.sugarcrm.com/download'
  }, {
    'name': 'Open Cases',
    'href': '//www.sugarcrm.com/support/portal'
  }, {
    'name': 'Bug Tracker',
    'href': 'https://web.sugarcrm.com/support/issues'
  }, {
    'name': 'On-Demand Status',
    'href': '//status.sugarcrm.com/'
  }
]);
...
},{"./widget":14}],13:[function(require,module,exports){
var Widget, toc;
...
toc.setTitle('Training & Help');

toc.contents([
  {
    'name': 'Community Forums',
    'href': 'https://community.sugarcrm.com/sugarcrm'
  }, {
    'name': 'Sugar University',
    'href': '/03_Training'
  }, {
    'name': 'Sugar Certification Program',
    'href': '/03_Training/Certification'
  }, {
    'name': 'Sugar 6.x to Sugar 7 Upgrade Training',
    'href': '/03_Training/06_Upgrade_Training/'
  }, {
    'name': 'Hot Topic Webinars',
    'href': '/03_Training/Webinars/Hot_Topic'
  }, {
    'name': 'Live Classes',
    'href': '//www.sugarcrm.com/university_classes/list'
  }, {
    'name': 'Knowledge Base',
    'href': '/04_Knowledge_Base'
  }, {
    'name': 'Developer Blog',
    'href': '//developer.sugarcrm.com/'
  }
]);
...
},{"./widget":14}],14:[function(require,module,exports){
var Widget;
...
Widget = (function() {
  function Widget(name) {
    ...
    this.setFn(this.list);
    return;
  }

  Widget.prototype.setTitle = function(text) {
    this.title = document.createElement('h2');
    this.title.innerHTML = text;
  };

  Widget.prototype.setDescription = function(text) {
    this.description = document.createElement('p');
    this.description.innerHTML = text;
  };

  Widget.prototype.contents = function(data) {
    this.data = data;
  };

  Widget.prototype.list = function(data) {
    var a, datum, icon, li, ul, _i, _len;
    ul = document.createElement('ul');
    for (_i = 0, _len = data.length; _i < _len; _i++) {
      ...
      if (datum && datum.href) {
        a.setAttribute('href', datum.href);
      }
      if (datum && datum.name) {
        a.innerHTML = datum.name;
      }
      if (datum && datum.target) {
        a.setAttribute('target', datum.target);
        if (datum.target === '_blank') {
  ...
  Widget.prototype.setFn = function(fn) {
    this.fn = fn;
  };

  Widget.prototype.build = function() {
    var c, html;
    html = this.fn(this.data);
    c = document.createElement('div');
    c.className = 'content';
    c.appendChild(this.title);
    c.appendChild(this.description);
    c.appendChild(html);
    this.element.appendChild(c);
    this.container = this.element;
  };

  Widget.prototype.append = function(id) {
    var d;
